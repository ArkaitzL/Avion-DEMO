...
public class Avion : MonoBehaviour
{
    ...
    [SerializeField] GameObject paquete;
    [SerializeField] Transform deposito_posicion;
    ...
    void Controles() 
    {
        ...
        //Soltar paquete
        if (Input.GetKeyDown(teclas.soltar_paquete) && ControladorBG.Esperando("Deposito")) {
            ControladorBG.IniciarEspera("Deposito", 2f);
            SoltarPaquete();
        } 
    }
    ...
    void SoltarPaquete() 
    {
        animacion.SetTrigger("Deposito");
        ControladorBG.Rutina(.75f, () => {      
            GameObject inst = Instantiate(paquete, deposito_posicion.position, Quaternion.identity);
    ...
    private void OnCollisionStay(Collision collision)
    {
        if (!enSuelo)
        {
            ...
            GameObject parte = collision.GetContact(0).thisCollider.gameObject;
            if (parte.CompareTag("Choque"))
            {
                // ** TEMPORAL **
                Debug.Log("Boom");
            }
